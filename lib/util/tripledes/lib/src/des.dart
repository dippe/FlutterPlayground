import 'package:todo_flutter_app/util/tripledes/lib/src/utils.dart';
import 'package:todo_flutter_app/util/tripledes/lib/tripledes.dart';
import 'package:todo_flutter_app/util/tripledes/lib/src/constants.dart';
...
class DESEngine extends BaseEngine {
  ...
  void init(bool forEncryption, List<int> key) {
    ...
    for (var i = 0; i < 56; i++) {
      ...
      keyBits[i] = (rightShift32(this.key[rightShift32(keyBitPos, 5)], (31 - keyBitPos % 32))) & 1;
    }
    ...
    for (var nSubKey = 0; nSubKey < 16; nSubKey++) {
      ...
      for (var i = 0; i < 24; i++) {
        ...
        subKey[(i ~/ 6) | 0] |= leftShift32(keyBits[((PC2[i] - 1) + bitShift) % 28], (31 - i % 6));

        // Select from the right 28 key bits
        subKey[4 + ((i ~/ 6) | 0)] |= leftShift32(keyBits[28 + (((PC2[i + 24] - 1) + bitShift) % 28)], (31 - i % 6));
      }
  ...
  void exchangeLR(offset, mask) {
    var t = (((rightShift32(this._lBlock, offset)).toSigned(32) ^ this._rBlock) & mask).toSigned(32);
    (this._rBlock ^= t).toSigned(32);
    this._lBlock ^= (t << offset).toSigned(32);
  }

  void exchangeRL(offset, mask) {
    var t = (((rightShift32(this._rBlock, offset)).toSigned(32) ^ this._lBlock) & mask).toSigned(32);
